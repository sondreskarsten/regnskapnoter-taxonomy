...
For each role-scoped calc arc, assemble parent ≈ Σ(weight × child) and
...
CONCEPT_TO_FINSTAT = {
    ...
    "regnskap-no:NedskrivningFinansielleEiendeler": "NedskrAnleggsmidler",
...
def validate_arcs(rows, tolerance: float = 0.05) -> tuple[int, int, list[str]]:
    arcs = load_calc_arcs()
